...
class CLTrainer(Trainer):
    ...
    def evaluate(
        ...
        for lang, eval_dataset in self.validation_datasets.items():
            ...
            #cos_sim = F.cosine_similarity(z1.unsqueeze(1), z2.unsqueeze(0), dim=-1)
    ...
    def _maybe_log_save_evaluate(self, loss, model, trial, epoch):
        if self.control.should_log:
            logs: Dict[str, float] = {}
            tr_loss_scalar = loss[0].item()
            rlm_loss_scalar = loss[1].item()
            tlm_loss_scalar = loss[2].item()
            # reset tr_loss to zero
            for l in loss:
                l -= l

            logs["tr_loss"] = round(tr_loss_scalar / (self.state.global_step - self._globalstep_last_logged), 4)
            logs["rlm_loss"] = round(rlm_loss_scalar / (self.state.global_step - self._globalstep_last_logged), 4)
            logs["tlm_loss"] = round(tlm_loss_scalar / (self.state.global_step - self._globalstep_last_logged), 4)
    ...
    def train(self, model_path: Optional[str] = None, trial: Union["optuna.Trial", Dict[str, Any]] = None):
        ...
        loss = [torch.tensor(0.0).to(self.args.device) for _ in range(3)]
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if ((step + 1) % self.args.gradient_accumulation_steps != 0) and self.args.local_rank != -1:
                    ...
                    with model.no_sync():
                        ...
                        loss = [loss[i] + output[i] for i in range(3)]
                else:
                    output = self.training_step(model, inputs)
                    loss = [loss[i] + output[i] for i in range(3)]
                ...
                if (step + 1) % self.args.gradient_accumulation_steps == 0 or (
                    ...
                    self._maybe_log_save_evaluate(loss, model, trial, epoch)
        ...
        self._total_loss_scalar += loss[0].item()
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]]) -> torch.Tensor:
        ...
        if self.args.gradient_accumulation_steps > 1:
            loss = [l / self.args.gradient_accumulation_steps for l in loss]  

        tr_loss = loss[0]
        if self.use_apex:
            with amp.scale_loss(tr_loss, self.optimizer) as scaled_loss:
                scaled_loss.backward()
        elif self.deepspeed:
            # calling on DS engine (model_wrapped == DDP(Deepspeed(PretrainedModule)))
            self.model_wrapped.module.backward(tr_loss)
        else:
            tr_loss.backward()

        loss = [l.detach() for l in loss]
